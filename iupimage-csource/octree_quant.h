 ...
 * Slightly modified to fit the need of iupimage-csource. The dithering
 * process doesn't play well for unknown reason.
 * According to its webpage it's under GNU Free Documentation License 1.2:
 * https://rosettacode.org/wiki/Color_quantization/C
 */

#include <stdio.h>
#ifndef	OCTREE_QUANT_H
#define OCTREE_QUANT_H
...
typedef struct {
	int alloc, n;
	oct_node* buf;
	oct_node  root;	/* AX20160527: storing the root pointer */
} node_heap;
...
node_heap *color_quant_octree_alloc(unsigned char *data, int pixels, int chan)
{
	node_heap	*heap;
	oct_node	got;
	int 		i;
	...
	if ((heap = calloc(sizeof(node_heap), 1)) == NULL) {
		return NULL;
	}
	heap->root = node_new(0, 0, 0);

	for (i = 0; i < pixels; i++, pix += chan)
		heap_add(heap, node_insert(heap->root, pix));
...
void color_quant_octree_free(node_heap *heap)
{
	if (heap) {
		node_free();
		free(heap->buf);
...
int color_quant_octree_indexing(node_heap *heap, unsigned char *pix)
{
	oct_node root = heap->root;
	int	n;
	unsigned char i, bit;
...
int color_quant_dither(node_heap *h, unsigned char *data, 
		unsigned char *outdata, int wid, int hei, int chan)
{
	oct_node nearest_color(int *v, int *idx) {
		int i;
...
#	define POS(i, j) (chan * ((i) * wid + (j)))
	int i, j, idx = 0;
	int *npx = calloc(sizeof(int), hei * wid * chan), *px;
	int v[3];
	unsigned char *pix = data;
	oct_node nd;
...
#define CTOTAL (C00 + C11 + C10 + C01)
	...
	for (px = npx, i = 0; i < hei; i++) {
		for (j = 0; j < wid; j++, pix += chan, px += chan) {
...
#define clamp(x, i) if (x[i] > 255) x[i] = 255; if (x[i] < 0) x[i] = 0
	...
	for (px = npx, i = 0; i < hei; i++) {
		...
		for (j = 0; j < wid; j++, pix += chan, px += chan) {
			...
			nd = nearest_color(px, &idx);
			//pix[0] = nd->r; pix[1] = nd->g; pix[2] = nd->b;
			*outdata++ = (unsigned char)idx;
